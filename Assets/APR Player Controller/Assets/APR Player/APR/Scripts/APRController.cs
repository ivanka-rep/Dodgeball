...
public class APRController : MonoBehaviour
{
    ...
    {
	    get => m_throwingController;
	    set => m_throwingController = value;
    }
    public JoyStick Joystick { get => m_joystick; set => m_joystick = value; }

    public float ControlHorizontal { get => m_controlHorizontal; set => m_controlHorizontal = value; }
    public float ControlVertical { get => m_controlVertical; set => m_controlVertical = value; }
    
    public Quaternion ThrowOriginRotation { get => m_throwOriginRotation; set => m_throwOriginRotation = value; }


    public bool isBot;
    
    [Header("Controllers")]
    [SerializeField] private ThrowingController m_throwingController;
    [SerializeField] private Bot m_bot;
    [SerializeField] private KnockOutAnimation m_nockAnim;
    [SerializeField] private JoyStick m_joystick;
    private float m_controlHorizontal;
    private float m_controlVertical;
    private Quaternion m_throwOriginRotation;
    ...
    public bool ResetPosition { get => ResetPose; set => value = ResetPose; }
	...
	Quaternion
	//
	RootTarget, HeadTarget, BodyTarget,
    ...
    void Update()
    {
        if( (useControls || isBot) && !inAir)
        {
	        PlayerMovement();
            
            if(canPunch && useControls && !isBot)
            {
                PlayerPunch();
            }
        }
    ...
    void PlayerSetup()
    {
		...
		RootTarget = APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation;
        ...
        m_throwingController.SetAprVars(APR_Parts);
    }  
    ...
    void PlayerMovement()
    {
	    if (!(useControls || isBot)) return;
	    
	    float playerMove = 0f;
	    if (useControls)
	    {
		    float horizontal = Input.GetAxisRaw(leftRight); 
		    float vertical = Input.GetAxisRaw(forwardBackward);
		    
		    if (m_joystick != null && m_joystick.Settings.IsTouched)
		    ...
		    if (horizontal != 0 && vertical != 0) playerMove = (horizontal + vertical) / 2f;
		    else playerMove = horizontal + vertical;
	    }
        ...
        {
	        if (playerMove != 0 || (isBot && m_bot.forwardBackward != 0) )
	        {
		        //Debug.Log(" move. ");
		        float moveForce = 0f;
		        if (useControls && !isBot) moveForce = playerMove;
		        if (isBot && !useControls) moveForce = m_bot.forwardBackward;

		        var v3 = APR_Parts[0].GetComponent<Rigidbody>().transform.forward * (moveForce * moveSpeed);
		        if ((m_controlHorizontal != 0f || m_controlVertical != 0f) && !isBot) 
		        { v3 = ( Vector3.forward * -m_joystick.Vertical + Vector3.right * -m_joystick.Horizontal ) * (moveForce * (moveSpeed * 1.1f)); }
		        
		        v3.y = APR_Parts[0].GetComponent<Rigidbody>().velocity.y;
		        APR_Parts[0].GetComponent<Rigidbody>().velocity = v3;
	        }
            
            if(useControls && playerMove > 0 || isBot && m_bot.forwardBackward > 0f )
            {
                if(!WalkForward && !moveAxisUsed)
                {
                    WalkBackward = false;
            ...
            else if(useControls && playerMove < 0 || isBot && m_bot.forwardBackward < 0f)
            {
            ...
            else if(playerMove == 0 || isBot && m_bot.forwardBackward == 0)
            {
    ...
    void PlayerRotation()
    {
        ...
        {   
            //Self Direction
            //Turn with keys
            float horizontal = Input.GetAxisRaw(leftRight), vertical = Input.GetAxisRaw(forwardBackward);
            
            if (m_joystick != null && m_joystick.Settings.IsTouched)
            {   horizontal += m_joystick.Horizontal; vertical += m_joystick.Vertical; }
            
            horizontal = horizontal > 1f ? 1f : horizontal;

            if ( (m_controlHorizontal != 0f || m_controlVertical != 0f ) && !isBot)
            {
	            Vector3 joyFacing = (Vector3.forward * m_controlVertical) + (Vector3.right * -m_controlHorizontal);  joyFacing.y = 0f;
	            APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation = Quaternion.Lerp 
		            (APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation, Quaternion.LookRotation(joyFacing), Time.fixedDeltaTime * (turnSpeed / 2f) );
            }
            else if ( (horizontal != 0 || vertical != 0 ) && !isBot)
            {
	            Vector3 joyFacing = (Vector3.forward * vertical) + (Vector3.right * -horizontal);  joyFacing.y = 0f;
	            APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation =
		            Quaternion.Slerp(APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation, Quaternion.LookRotation(joyFacing), Time.deltaTime * turnSpeed);
            }
            else if (isBot && m_bot.leftRight != 0 && !m_bot.isNeedAim)
            {
	            Quaternion targetRot = new Quaternion(APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation.x, APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation.y - (m_bot.leftRight * turnSpeed),
		            APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation.z, APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation.w);
	            APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation = Quaternion.Lerp (APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation, targetRot, 6 * Time.fixedDeltaTime);
            }
			// set bot's rotation to required lookAt GameObject
            else if (isBot && m_bot.isNeedAim && m_bot.lookAtTransform != null)
            {
	            //Debug.Log("AIM TO OBJECT " + m_bot.lookAtTransform);
	            Vector3 dir = m_bot.lookAtTransform.position - APR_Parts[0].transform.position; dir.y = 0f;
	             APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation = Quaternion.Lerp(APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation,
		             Quaternion.Inverse(Quaternion.LookRotation(dir)), 1f);
            }
    ...
    void PlayerReach()
    {
        ...
        //Reach Left
        if ( false && Input.GetAxisRaw(reachLeft) != 0 &&  !punchingLeft && !m_throwingController.throwLeft)
        { 
        ...
        if(m_throwingController.reachLeft && !punchingLeft)
        {
            if(reachLeftAxisUsed)
            {
                if(balanced)
        ...
        //Reach Right
        if( false && Input.GetAxisRaw(reachRight) != 0 && !punchingRight && !m_throwingController.throwRight)
        {
        ...
        if(m_throwingController.reachRight && !punchingRight)
        {
            if(reachRightAxisUsed)
            {
                if(balanced)
    ...
    //---Reset Player Pose---//
    //////////////////////////
	public void ResetPlayerPose(bool immediateCall = false)
    {
        if(ResetPose || immediateCall && !jumping)
        {
	...
	// private Quaternion CalculateJointRotation(ConfigurableJoint joint, Quaternion startRotation, Quaternion targetRotation)
	// {
	...
	// 	
	// 	// get our newly calculated rotation
